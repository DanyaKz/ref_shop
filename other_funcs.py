...
class Other_Funcs(Qiwi_init):
    ...
    async def before_check(self,user_id,bill):
        check = self.check_payment(user_id,bill)
    ...
    async def to_pay(self, user_id , amount):
        ...
        print(len(get_data['all_parents']) >= get_data['user_lvl'] and len(get_data['all_parents']) != 0)
            ...
            if parent_coin_lvl >= get_data['user_lvl'] :
                ...
                if get_data['user_lvl'] == 0:
                    ...
                    if not self.pay_parent(parent['user_id'],amount/2):
                        await self.bot.send_message(parent['user_id'],
                        'К сожалению, деньги не получилось отправить, но они были сохранены и утром бот попробует отправить их снова, а пока проверь правильность своего номера, который находится в «меню» - «мои данные», либо же проверь лимиты своего кошелька  https://qiwi.com/settings/identification',
                         disable_web_page_preview=True)
            else:
                mark = InlineKeyboardMarkup().add(InlineKeyboardButton('Повысить уровень', url="https://t.me/SIYP_ideas/7"))
                await self.bot.send_message(parent['user_id'],
                    f'Твой реферал купил уровень, а ты с этого не заработал ☹️\nПочему? 🤔\nТы не получаешь бонусы с «{parent_coin_lvl+1}» уровня. 😢 \nПовысь уровень и не упускай свой заработок 😉',
                    reply_markup = mark)
        else : 
    ...
    def combine_clidren_1_lvl(self, user_id):
        ...
        if len(children) > 0:
            for i in range(len(children)):
                if children[i]['username'] != None:
                    to_return += f"""<i><a href = "https://t.me/{children[i]['username']}">«{children[i]['first_name']}»</a></i>,"""
                else:
                    to_return += f"""<i>«{children[i]['first_name']}»</i>,"""
        return to_return[:-1]
    ...
    async def my_children(self, user_id):
        ...
        msg += f"\n💡 Подсказка:\nТвой уровень – «{get_user['lvl']}».\nТы получаешь 50% - с 1 по «{get_user['lvl']}» ур. \n\n"
        # print("bool(children_plus_lvl['floor'])",bool(children_plus_lvl['floor']))
        if bool(children_plus_lvl['floor']):
            msg += f"На «{get_user['lvl']+1}» уровне у тебя – «{children_plus_lvl['c']}» реф.\nВозможный доход от уровня – «{children_plus_lvl['c']*50}» руб. \nСоветую повысить уровень."

        msg += '</i>'
        print(msg)
        mark_up = {"inline_keyboard":[[{"text":f"Приобрести Бизнес-идею ","url":"https://t.me/SIYP_ideas/7"}],
    ...
    async def my_salary(self, user_id):
        ...
        msg = f"На твоём балансе - {get_user['balance']} руб.\nБот вывел - {salary} руб.\n\n<b><u>Всего - {get_user['balance']+salary} руб.</u></b>\n\nТы пригласил всего - {children} реф. \n\n{'<b><u>Проверь лимиты кошелька!</u></b>' if get_user['balance']!= 0 else ''}"
    ...
    async def daily_users_response(self):
        ...
        lvl_up = InlineKeyboardButton('Повысить уровень', url='https://t.me/SIYP_ideas/7')
    ...
    async def daily_zero_users_response(self):
        users = self.users_over_lower_1_lvl('<')
        lvl_up = InlineKeyboardButton('Бизнес-идеи', url='https://t.me/SIYP_ideas/7')
